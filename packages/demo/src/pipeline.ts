/**
 * @attestia/demo — pipeline walkthrough.
 *
 ...
 *
 * Uses real domain packages directly (no HTTP server). This module performs the
 * heavy workspace imports; the thin `index.ts` bootstrap imports it dynamically
 * so a missing `dist/` build fails with a branded hint instead of a raw stack.
 */
...
export interface RunOptions {
  /** Collapse the ~10s walkthrough to instant (no per-step pacing). */
  readonly fast: boolean;
}

function makeSleep(delayMs: number): (ms: number) => Promise<void> {
  return (ms: number) => {
    const effective = delayMs === 0 ? 0 : ms;
    if (effective <= 0) return Promise.resolve();
...
/** Inner width (in columns) of the banner box — every row is built to this width. */
const BANNER_WIDTH = 58;

/**
 * Render one centred row of the banner box at exactly {@link BANNER_WIDTH}
 * inner columns, framed by the cyan ║ borders. Centring + padding are computed
 * from `text.length`, so the right edge always lines up regardless of the text.
 */
function boxLine(text: string, paint: (s: string) => string): string {
  const inner = text.length > BANNER_WIDTH ? text.slice(0, BANNER_WIDTH) : text;
  const totalPad = BANNER_WIDTH - inner.length;
  const left = Math.floor(totalPad / 2);
  const right = totalPad - left;
  const row = " ".repeat(left) + inner + " ".repeat(right);
  return chalk.cyan.bold("  ║") + paint(row) + chalk.cyan.bold("║");
}

function banner(): void {
  const top = chalk.cyan.bold("  ╔" + "═".repeat(BANNER_WIDTH) + "╗");
  const bottom = chalk.cyan.bold("  ╚" + "═".repeat(BANNER_WIDTH) + "╝");
  console.log();
  console.log(top);
  console.log(boxLine("ATTESTIA DEMO", (s) => chalk.white.bold(s)));
  console.log(boxLine("Financial Truth Infrastructure", (s) => chalk.gray(s)));
  console.log(bottom);
  console.log();
}

/** A single dim "why it matters" line under a conceptual step header. */
function why(msg: string): void {
...
export async function run(opts: RunOptions): Promise<void> {
  const DELAY_MS = opts.fast ? 0 : 600;
  const sleep = makeSleep(DELAY_MS);

  banner();
  console.log(chalk.gray("  Walk-through of the full Attestia pipeline."));
  console.log(chalk.white("  You'll watch one payroll payment become an independently"));
  ...
  if (opts.fast) {
    console.log(chalk.dim("\n  Running in fast mode (pacing disabled).\n"));
  } else {
    console.log(chalk.dim("\n  Paced for reading (~10s, auto-advancing). Use --fast to skip waits.\n"));
  }
  ...
  why("Does what we INTENDED match what the LEDGER recorded and what actually happened ON-CHAIN?");
  ...
  why("One root hash commits to every event — change any one and the root no longer matches.");
  ...
  why("Anyone can verify this attestation offline — no need to trust or contact Attestia.");
  ...
  why("Recompute every hash from the package alone — this is what an external auditor runs.");
