...
>
> **Dependency-advisory posture (2026-06-21, cycle-2):** `pnpm audit --prod --audit-level=high` is **clean (0 high)**. The newly-published `hono` CORS advisory (GHSA-88fw-hqm2-52qc — wildcard-origin reflection with credentials, fixed `>=4.12.25`) is **RESOLVED** — `packages/node` bumped `hono@^4.12.4` → `hono@^4.12.25`; the CI dep-scan caught it on the v2.0.0 release PR and the bump cleared it. The earlier Stage A `hono` / `@hono/node-server` authorization-bypass advisories (GHSA-q5qw-h33p-qvwr, GHSA-wc8c-qw6v-h7f6) remain resolved (`@hono/node-server@^1.19.10`). `pnpm audit --prod` (moderate+) now reports **1 moderate**, all transitive in the read-only Solana RPC path under `@attestia/chain-observer` → `@solana/web3.js` (already at latest):
